...
#SBATCH --export=NONE
#SBATCH --exclusive
#SBATCH -o /data3/marine_diseases_lab/erin/2017_2020_Transcriptome_Analysis/pipeline_files/2020_Scripts/Script_out_error_files/MAFFT_RAxML_IAP_5_22_2020
#SBATCH -e /data3/marine_diseases_lab/erin/2017_2020_Transcriptome_Analysis/pipeline_files/2020_Scripts/Script_out_error_files/MAFFT_RAxML_IAP_error_5_22_2020
...
array1=($(cat $F/BIR_IAP_HMMER_Interpro_XP_list_all.txt))
for i in ${array1[@]}; do
	sed -n "/${i}/,/^>/p" $O/All_mollusc_prot.faa | sed '$d' >> $F/BIR_IAP_HMMER_Interpro_XP_list_all.fa
	echo "done"
done

echo "fetch sequences done $(date)"

## Remove duplicates of sequences that are identical
...
# -t is redundancy level, tolerate 1
# -c is sequence identity, set identity to 1.0
...
echo "Start MAFFT all mollusc IAP"
cd $M/
mafft --auto --thread 20 $F/BIR_IAP_HMMER_Interpro_XP_list_all_rm_dup.fa > $M/BIR_IAP_HMMER_Interpro_XP_list_all_MSA.fa
...
#echo "Start RAxML all mollusc IAP"
#cd $R/
#mpiexec raxmlHPC-MPI-AVX -s $M/BIR_IAP_HMMER_Interpro_XP_list_all_MSA.fa -n BIR_IAP_HMMER_Interpro_XP_list_all_MSA_RAxML -m PROTGAMMAAUTO -x 12345 -p 12345 -f a -N autoMRE
#echo "DONE RaxML all mollusc IAP"
