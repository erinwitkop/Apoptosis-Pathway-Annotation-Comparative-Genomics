...
# Are any XPs shared
# Hit to ive orthogroups: "OG0000042", "OG0001474", "OG0006793", "OG0016185" ,"OG0029406"
...
# Hit to 8 orthgroups: "OG0000042" "OG0001474" "OG0006793" "OG0011177" "OG0011415" "OG0012261" "OG0019697" "OG0024688"
...
IAP_ALL_XP_lookup$Orthogroups
# 151 orthogroups
# Are any XPs shared
# Hit to ive orthogroups:
...
## CHECK FOR DUPLICATE PROTEIN HITS IN TWO SEPARATE ORTHOGROUPS TO ASSESS NEED FOR MERGE ##
# Parse and trimws
IAPC <- strsplit(IAP_CVlookup$Crassostrea_virginica, split = ",")
TLRC_parse <-data.frame(Orthogroup = rep(TLR_CVlookup$Orthogroup, sapply(TLRC, length)), protein_id = unlist(TLRC))
# trimws for checking list later 
TLRC_parse$protein_id <- trimws(TLRC_parse$protein_id, which=c("left"))
...
## CHECK FOR NON MAPPED PROTEINS FROM INITIAL CVIR CGIG ####
# Were any C_vir or C_gig TLR annotated proteins NOT mapped to Orthogroups?
IAP_CV_notmapped <- left_join(IAP_XP_CV, IAPC_parse, by ="protein_id") %>% filter(is.na(Orthogroup)) %>% View() # Three were not mapped to Orthogroups (need to check these and add to counts list)
